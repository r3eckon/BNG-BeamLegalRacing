...
extensions.blrpartmgmt.setSlot(p["slot"], p["item"])						
end
...
local filter = p
extensions.blrpartmgmt.setFilter(filter)
...
list = extensions.blrpartmgmt.getVehicleParts()
extensions.customGuiStream.sendDataToUI("usedParts", list)
list = extensions.blrpartmgmt.getSlotNameLibrary()
...
list = extensions.blrpartmgmt.getAdvancedInventoryUIParts()
list = extensions.blrpartmgmt.advancedInventorySearch(list)
extensions.customGuiStream.sendDataToUI("advinvFilter", list)
...
list = extensions.blrpartmgmt.getSlotNameLibrary()
extensions.customGuiStream.sendDataToUI("slotNames", list)
list = extensions.blrpartmgmt.getSortedGarageSlots()
extensions.customGuiStream.sendDataToUI("sortedGarageSlots", list)
list = extensions.blrpartmgmt.getSortedGarageParts()
...
if p == 0 then
list = extensions.blrpartmgmt.getPartShopList()
list = extensions.blrpartmgmt.categoryFilter(list, true)
...
list = extensions.blrpartmgmt.getTuningUIData()
extensions.customGuiStream.sendDataToUI("tuningData", list)
list = extensions.blrpartmgmt.getTuningUIValues()
...
list = extensions.blrpartmgmt.getSortedTuningCategories(false)
extensions.customGuiStream.sendDataToUI("tuningSortedCategories", list)
list = extensions.blrpartmgmt.getSortedTuningFields(false)
...
local cmap = extensions.blrpartmgmt.getChildMap()
local pmap = extensions.blrpartmgmt.getParentMap()
extensions.customGuiStream.sendRepairUIMaps(pmap, cmap)
local mainpart = extensions.blrpartmgmt.getMainPartChild()
...
local ilinkodo = extensions.blrpartmgmt.getInventoryLinkOdometers(true)
...
local fidlist = extensions.blrpartmgmt.getSlotKeyedFullInventory()
local fidsortedslots = extensions.blrpartmgmt.getSortedFullInventorySlots()
local fidslotnames = extensions.blrpartmgmt.getFullSlotNameLibrary()
local fidpartnames = extensions.blrpartmgmt.getFullPartNameLibrary()
extensions.customGuiStream.sendDataToUI("fidInventory",fidlist)
extensions.customGuiStream.sendDataToUI("fidSortedSlots",fidsortedslots)
...
-- 1.18 slot paths to slot id map
local slotPathIDMap = extensions.blrpartmgmt.getPathKeyedSlots()
extensions.customGuiStream.sendDataToUI("slotPathIDMap", slotPathIDMap)

-- 1.18 vehicle slot IDs list, for part buying compatibility indicator
local vehSlotIDsList = extensions.blrpartmgmt.getVehicleSlotIDsList()
extensions.customGuiStream.sendDataToUI("vehSlotIDs", vehSlotIDsList)

-- 1.18 vehicle installed parts list, for part buying "installed" indicator
local installedPartsList = extensions.blrpartmgmt.getVehicleInstalledPartsList()
extensions.customGuiStream.sendDataToUI("vehInstParts", installedPartsList)
...
local dtable = extensions.blrpartmgmt.tuningTableFromUIData(p, false)
...
extensions.blrpartmgmt.createTemplateFile(configPath, templatePath)
extensions.blrutils.uiRefreshTemplates()
end
...
local inventory = extensions.blrpartmgmt.getPartInventory()
extensions.customGuiStream.sendDataToUI("ownedParts", inventory)
local list = extensions.blrpartmgmt.getGarageUIData()
if searching then
list = extensions.blrpartmgmt.searchFilter(list, true, true)
else
list = extensions.blrpartmgmt.categoryFilter(list, true)
...
extensions.blrpartmgmt.setSlotDelayedNoInventory(v, "")
...
extensions.blrpartmgmt.setSlotDelayedNoInventory(engineSlot, "")
...
-- 1.18 fix, now checking if child part to remove due to unrepaired damaged parent was selected for repair
if ((not damage[part]) or p[part]) and part~="" then
...
-- 1.18 fix, same as explained above
if (damage[part] and(not p[part])) and part~="" then
...
-- 1.18 fix to send updated owned/used part data to UI after repair is done
extensions.blrglobals.blrFlagSet("uiInitRequest", true)
end
...
extensions.blrpartmgmt.setSlotWithChildren(p["slot"], p["part"], p["pid"])
end
...
cpart = k:sub(5)
ilinks[cpart] = v .. ",1234567890" -- odo doesnt matter, will be updated when template loads
...
extensions.blrpartmgmt.setCacheValidBypass(true) -- bypass valid cache to force update
...
extensions.blrpartmgmt.loadFavorites() -- reload new favorites
end
