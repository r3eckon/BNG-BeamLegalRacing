...
toSet = extensions.betterpartmgmt.getVehicleData().chosenParts
toSet[pslot] = iparent
...
parts = extensions.betterpartmgmt.getAvailablePartList()[slot]
loadParts=false
...
pparts = {}
local ptemp = extensions.betterpartmgmt.getAvailablePartList()[pslot]
local pcurr = extensions.betterpartmgmt.getVehicleData().chosenParts[pslot]
iparent = pcurr
...
if _gpslot then
gpparts = {}
ptemp = extensions.betterpartmgmt.getAvailablePartList()[gpslot]
pcurr = extensions.betterpartmgmt.getVehicleData().chosenParts[gpslot]
...
for k,v in pairs(extensions.betterpartmgmt.getChildMap()[gpparts[cgparent]]) do
if string.find(k, gkeyword) then
pslot = k
logfile("Setting pslot to " .. k)
sfound = true
pparts = extensions.betterpartmgmt.getAvailablePartList()[pslot]
break
end
end
end
...
if extensions.betterpartmgmt.getChildMap()[pparts[cparent]] then
for k,v in pairs(extensions.betterpartmgmt.getChildMap()[pparts[cparent]]) do
...
parts = extensions.betterpartmgmt.getAvailablePartList()[cslot]
